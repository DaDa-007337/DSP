...
volatile struct SCI_REGS *pSPI = &SciaRegs;
...
interrupt void uartRx_isr(void)
{
	...
	{
		if(SCI_Msg.Mark_Para.Status_Bits.DISRevflag == 0)
		{
...
{
	InitSciaGpio();
	pSPI->SCICTL1.bit.SWRESET = 0;

	pSPI->SCICCR.all =0x0007;   // 1 stop bit,  No loopback
                                   // No parity,8 char bits,
                                   // async mode, idle-line protocol
// baud = LSPCLK/8/((BRR+1) 
// baud @LSPCLK = 15MHz (60 MHz SYSCLK)
	pSPI->SCIHBAUD    =0x0000;
	pSPI->SCILBAUD    =14;      //0XC2-->9600 ; 97--> 19200 ;0x30-->38400;14-->128000

	pSPI->SCICTL1.bit.SWRESET = 1;     // Relinquish SCI from Reset
	pSPI->SCIFFTX.bit.SCIRST=1;

	pSPI->SCIFFRX.bit.RXFFIL  = SCI_FIFO_LEN;  //设置FIFO深度
	pSPI->SCICTL1.bit.TXENA = 1;       //使能发送
	pSPI->SCICTL1.bit.RXENA = 1;       //使能接收

//	SciaRegs.SCICTL2.bit.TXINTENA =1;
//	SciaRegs.SCICTL2.bit.RXBKINTENA =1;
//  SciaRegs.SCIFFTX.bit.TXFFIENA = 0; //禁止发送中断使能
	//中断配置步骤-----1
	pSPI->SCIFFTX.bit.SCIFFENA = 1; //使能FIFO中断
	pSPI->SCIFFRX.bit.RXFFIENA=1;
	...
	pSPI->SCIFFCT.all=0x00;
	pSPI->SCIFFTX.bit.TXFIFOXRESET=1;
	pSPI->SCIFFRX.bit.RXFIFORESET=1;
}
